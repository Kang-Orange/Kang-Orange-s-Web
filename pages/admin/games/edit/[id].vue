...
// Developers (制作组/制作人员)
const developerList = ref<{ name: string; role: string; id?: number }[]>([])
function addDeveloper() {
  developerList.value.push({ name: '', role: '' })
}
...
if (!isNew.value) {
  ...
  watch(data, (entry) => {
    ...
    if (found) {
      ...
      developerList.value = (found.developers || []).map((d: any) => ({ name: d.name, role: d.role || '', id: d.id }))
...
async function save() {
  ...
  try {
    ...
    const payload = {
      ...
      developers: developerList.value,
    }
...
<template>
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <form v-else @submit.prevent="save" class="space-y-6">
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <label class="text-gray-400 text-sm">制作组 / 制作人员</label>
          <button
            type="button"
            @click="addDeveloper"
        ...
        <div v-if="developerList.length === 0" class="text-gray-600 text-xs">暂无</div>
        <div v-else class="space-y-2">
          <div v-for="(dev, i) in developerList" :key="i" class="flex items-center gap-2">
            <input
              v-model="dev.name"
              type="text"
              placeholder="名称"
            ...
            <input
              v-model="dev.role"
              type="text"
              placeholder="角色，如：剧本、原画"
              class="w-40 px-3 py-1.5 bg-gray-800 border border-gray-700 rounded-lg text-gray-200 placeholder-gray-500 focus:outline-none focus:border-indigo-500/50 text-sm"
            />
            <button
              type="button"
    ...
    <CropModal
      ...
      :src="cropSource"
      :aspectRatio="useCoverConfig().gameAspectRatio"
      @crop="onCropConfirm"
      @cancel="onCropCancel"
    />
  </div>
